...
helm upgrade -i ms-account-service social-coach-stable/social-coach-ms-service-sql-ng --namespace github -f /ms-account-service/values.yaml -f /ms-account-service/dev.yaml --reset-values --set image.tag=latest